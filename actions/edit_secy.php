	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		if(isset($_POST['update'])){
			...
			if(mysqli_num_rows($request)>0){
				$_SESSION['edit_secy_message'] = "Secretary exists!";
			}else{
				$rset_flag = sha1(rand(10,100));
				// echo $rset_flag;
				$sql = "INSERT INTO users (hostel, name, username, email, phone, level, rset_flag) VALUES ('" . $_POST['hostel'] . "','" . $_POST['name'] . "','" . $_POST['username'] . "','" . $_POST['email'] . "','" . $_POST['phone'] . "', 'secy', '$rset_flag')" ;
				$request = mysqli_query($conn, $sql);
				$_SESSION['edit_secy_message'] = "Secretary added.";

				$sql = "SELECT * FROM users WHERE hostel='" . $_POST['hostel'] . "' and name='" . $_POST['name'] . "' and username='" . $_POST['username'] . "' and email='" . $_POST['email'] . "' and phone='" . $_POST['phone'] . "'";
				$request = mysqli_query($conn, $sql);
				$newSecy = mysqli_fetch_array($request);
				...
				$mail->addAddress($newSecy['email'], $newSecy['name']);
				...
				$mail->Body = "Hello " . $newSecy['name'] . ",<br /> <a href='http://fromabctill.xyz/iitdh/'>Court</a> is a hostel complaint management portal.<br /><br />An account for you has been created by " . $_SESSION['name'] . ", following are the details:<br />username: '" . $newSecy['username'] . "'<br />Password: Click <a href='" . $link . "'>here</a> to set a password.<br /><br />Court";
				$mail->AltBody = "Hello " . $newSecy['name'] . ",<br /> <a href='http://fromabctill.xyz/iitdh/'>Court</a> is a hostel complaint management portal.<br /><br />An account for you has been created by " . $_SESSION['name'] . ", following are the details:<br />username: '" . $newSecy['username'] . "'<br />Password: Click <a href='" . $link . "'>here</a> to set a password.<br /><br />Court";
